...
export const getAppointmentById = async (id: string): Promise<AppointmentProps | null> => {
  try {
...
export const deleteAppointmentById = async (id: string): Promise<AppointmentProps | null> => {
  try {
    const { data } = await api.delete("/appointment/delete", {
      params: { _id: id },
    });
    if (!data) {
      return null;
